  ...
  wind::update ()
{
    ...
    if (dParticles > maxParticles) {
        if (verts) free(verts);
        if (colors) free(colors);
        if (texcoords) free(texcoords);
        maxParticles = dParticles * 2; // allocate a bit more
        verts = (float*)malloc(maxParticles * 4 * 3 * sizeof(float));
        colors = (float*)malloc(maxParticles * 4 * 3 * sizeof(float));
        texcoords = (float*)malloc(maxParticles * 4 * 2 * sizeof(float));
    }
	...
	case 0:		// lights
	{
		...
		float tempSize = 0.02f * float (dSize);
		for (i = 0; i < dParticles; i++) {
			float px = particles[i][0], py = particles[i][1], pz = particles[i][2];
			float r = particles[i][3], g = particles[i][4], b = particles[i][5];
			
			// Vertex 1 (bottom left)
			texcoords[t_idx++] = 0.0f; texcoords[t_idx++] = 0.0f;
			colors[c_idx++] = r; colors[c_idx++] = g; colors[c_idx++] = b;
			verts[v_idx++] = px - tempSize; verts[v_idx++] = py - tempSize; verts[v_idx++] = pz;
			
			// Vertex 2 (bottom right)
			texcoords[t_idx++] = 1.0f; texcoords[t_idx++] = 0.0f;
			colors[c_idx++] = r; colors[c_idx++] = g; colors[c_idx++] = b;
			verts[v_idx++] = px + tempSize; verts[v_idx++] = py - tempSize; verts[v_idx++] = pz;
			
			// Vertex 3 (top right)
			texcoords[t_idx++] = 1.0f; texcoords[t_idx++] = 1.0f;
			colors[c_idx++] = r; colors[c_idx++] = g; colors[c_idx++] = b;
			verts[v_idx++] = px + tempSize; verts[v_idx++] = py + tempSize; verts[v_idx++] = pz;
			
			// Vertex 4 (top left)
			texcoords[t_idx++] = 0.0f; texcoords[t_idx++] = 1.0f;
			colors[c_idx++] = r; colors[c_idx++] = g; colors[c_idx++] = b;
			verts[v_idx++] = px - tempSize; verts[v_idx++] = py + tempSize; verts[v_idx++] = pz;
		}
		...
		glDrawArrays(GL_QUADS, 0, dParticles * 4);
	...
	case 1:		// points
	{
		int v_idx = 0, c_idx = 0;
		for (i = 0; i < dParticles; i++) {
			colors[c_idx++] = particles[i][3];
			colors[c_idx++] = particles[i][4];
			colors[c_idx++] = particles[i][5];
			verts[v_idx++] = particles[i][0];
			verts[v_idx++] = particles[i][1];
			verts[v_idx++] = particles[i][2];
		}
		glPointSize (pointsize * 40.0f);
		glEnableClientState(GL_VERTEX_ARRAY);
		glEnableClientState(GL_COLOR_ARRAY);
		glVertexPointer(3, GL_FLOAT, 0, verts);
		glColorPointer(3, GL_FLOAT, 0, colors);
		glDrawArrays(GL_POINTS, 0, dParticles);
		glDisableClientState(GL_COLOR_ARRAY);
		glDisableClientState(GL_VERTEX_ARRAY);
		break;
	}
	case 2:		// lines
	{
		...
		for (i = 0; i < dParticles; i++) {
			if (linelist[i][1] >= 0) {
				// Vertex 1
				if (linelist[i][0] == -1) {
					colors[c_idx++] = 0.0f; colors[c_idx++] = 0.0f; colors[c_idx++] = 0.0f;
				} else {
					colors[c_idx++] = particles[i][3]; colors[c_idx++] = particles[i][4]; colors[c_idx++] = particles[i][5];
				}
				verts[v_idx++] = particles[i][0]; verts[v_idx++] = particles[i][1]; verts[v_idx++] = particles[i][2];
				...
				if (linelist[p2][1] == -1) {
					...
					colors[c_idx++] = particles[p2][3]; colors[c_idx++] = particles[p2][4]; colors[c_idx++] = particles[p2][5];
				}
				verts[v_idx++] = particles[p2][0]; verts[v_idx++] = particles[p2][1]; verts[v_idx++] = particles[p2][2];
				
				lineCount++;
			}
		}
		glLineWidth (linesize * 40.0f);
		glEnableClientState(GL_VERTEX_ARRAY);
		glEnableClientState(GL_COLOR_ARRAY);
		glVertexPointer(3, GL_FLOAT, 0, verts);
		glColorPointer(3, GL_FLOAT, 0, colors);
