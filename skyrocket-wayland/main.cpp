...
static char *fake_argv[] = { (char*)"skyrocket", NULL };
...
static void render_frame() {
    ...
    hack_draw(&xstuff, current_time, real_dt);
...
int main(int argc, char **argv) {
    ...
    if (!g_display) {
        fprintf(stderr, "skyrocket: cannot connect to Wayland display\n");
        return 1;
    }
    ...
    if (!g_compositor || !g_wm_base) {
        fprintf(stderr, "skyrocket: missing required Wayland globals\n");
        return 1;
    }
    ...
    xdg_toplevel_set_title(g_toplevel, "Skyrocket");
    xdg_toplevel_set_app_id(g_toplevel, "skyrocket");
