...
extern unsigned int smokelist[5], smoketex[5];
extern unsigned int flarelist[4], flaretex[4];
GLuint get_tex_for_list(GLuint dl) {
    for (int i = 0; i < 5; i++) if (smokelist[i] == dl) return smoketex[i];
    for (int i = 0; i < 4; i++) if (flarelist[i] == dl) return flaretex[i];
    return 0;
}
...
void draw_all_particles(std::vector<particle>& parts) {
    if (parts.empty()) return;
    
    std::map<GLuint, std::vector<PBV> > smokeBatches;
    ...
    for (size_t i = 0; i < parts.size(); i++) {
        particle* p = &parts[i];
        if (p->life <= 0.0f || p->type == POPPER || (p->depth < 0.0f && p->type != SHOCKWAVE)) {
            continue;
        }
        ...
        GLuint dl = p->displayList;
        float px = p->xyz[0], py = p->xyz[1], pz = p->xyz[2];
        float sz = p->size;
        float br = p->bright;
        
        float sr = 1.0f, sg = 1.0f, sb = 1.0f;
        if (p->type == SMOKE || p->type != EXPLOSION) {
            sr = p->rgb[0]; sg = p->rgb[1]; sb = p->rgb[2];
        }

        // define add_quad lambda manually since old g++ might not like auto lambdas, but we are using g++ which defaults to gnu++14 usually.
        // Actually I'll just write it inline to be safe.
        
        if (p->type == SMOKE) {
            float s = sz * 0.5f;
            float rx = right[0]*s, ry = right[1]*s, rz = right[2]*s;
            float ux = up[0]*s, uy = up[1]*s, uz = up[2]*s;
            smokeBatches[dl].push_back({px - rx - ux, py - ry - uy, pz - rz - uz, sr, sg, sb, br, 0.0f, 0.0f});
            smokeBatches[dl].push_back({px + rx - ux, py + ry - uy, pz + rz - uz, sr, sg, sb, br, 1.0f, 0.0f});
            smokeBatches[dl].push_back({px + rx + ux, py + ry + uy, pz + rz + uz, sr, sg, sb, br, 1.0f, 1.0f});
            smokeBatches[dl].push_back({px - rx + ux, py - ry + uy, pz - rz + uz, sr, sg, sb, br, 0.0f, 1.0f});
        } else if (p->type == EXPLOSION) {
            float s = sz * br * 0.5f;
            float rx = right[0]*s, ry = right[1]*s, rz = right[2]*s;
            float ux = up[0]*s, uy = up[1]*s, uz = up[2]*s;
            flareBatches[dl].push_back({px - rx - ux, py - ry - uy, pz - rz - uz, 1.f, 1.f, 1.f, br, 0.0f, 0.0f});
            flareBatches[dl].push_back({px + rx - ux, py + ry - uy, pz + rz - uz, 1.f, 1.f, 1.f, br, 1.0f, 0.0f});
            flareBatches[dl].push_back({px + rx + ux, py + ry + uy, pz + rz + uz, 1.f, 1.f, 1.f, br, 1.0f, 1.0f});
            flareBatches[dl].push_back({px - rx + ux, py - ry + uy, pz - rz + uz, 1.f, 1.f, 1.f, br, 0.0f, 1.0f});
        } else {
            float s1 = sz * 0.5f;
            float rx1 = right[0]*s1, ry1 = right[1]*s1, rz1 = right[2]*s1;
            float ux1 = up[0]*s1, uy1 = up[1]*s1, uz1 = up[2]*s1;
            flareBatches[dl].push_back({px - rx1 - ux1, py - ry1 - uy1, pz - rz1 - uz1, sr, sg, sb, br, 0.0f, 0.0f});
            flareBatches[dl].push_back({px + rx1 - ux1, py + ry1 - uy1, pz + rz1 - uz1, sr, sg, sb, br, 1.0f, 0.0f});
            flareBatches[dl].push_back({px + rx1 + ux1, py + ry1 + uy1, pz + rz1 + uz1, sr, sg, sb, br, 1.0f, 1.0f});
            flareBatches[dl].push_back({px - rx1 + ux1, py - ry1 + uy1, pz - rz1 + uz1, sr, sg, sb, br, 0.0f, 1.0f});

            float s2 = sz * 0.35f * 0.5f;
            float rx2 = right[0]*s2, ry2 = right[1]*s2, rz2 = right[2]*s2;
            float ux2 = up[0]*s2, uy2 = up[1]*s2, uz2 = up[2]*s2;
            flareBatches[dl].push_back({px - rx2 - ux2, py - ry2 - uy2, pz - rz2 - uz2, 1.f, 1.f, 1.f, br, 0.0f, 0.0f});
            flareBatches[dl].push_back({px + rx2 - ux2, py + ry2 - uy2, pz + rz2 - uz2, 1.f, 1.f, 1.f, br, 1.0f, 0.0f});
            flareBatches[dl].push_back({px + rx2 + ux2, py + ry2 + uy2, pz + rz2 + uz2, 1.f, 1.f, 1.f, br, 1.0f, 1.0f});
            flareBatches[dl].push_back({px - rx2 + ux2, py - ry2 + uy2, pz - rz2 + uz2, 1.f, 1.f, 1.f, br, 0.0f, 1.0f});
        }
    }
    ...
    glBlendFunc (GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
    for(std::map<GLuint, std::vector<PBV> >::iterator it = smokeBatches.begin(); it != smokeBatches.end(); ++it) {
        if(it->second.empty()) continue;
        glBindTexture(GL_TEXTURE_2D, get_tex_for_list(it->first));
        glVertexPointer(3, GL_FLOAT, sizeof(PBV), &it->second[0].x);
        glColorPointer(4, GL_FLOAT, sizeof(PBV), &it->second[0].r);
        glTexCoordPointer(2, GL_FLOAT, sizeof(PBV), &it->second[0].u);
        glDrawArrays(GL_QUADS, 0, it->second.size());
    }
    
    glBlendFunc (GL_SRC_ALPHA, GL_ONE);
    for(std::map<GLuint, std::vector<PBV> >::iterator it = flareBatches.begin(); it != flareBatches.end(); ++it) {
        if(it->second.empty()) continue;
        glBindTexture(GL_TEXTURE_2D, get_tex_for_list(it->first));
        glVertexPointer(3, GL_FLOAT, sizeof(PBV), &it->second[0].x);
        glColorPointer(4, GL_FLOAT, sizeof(PBV), &it->second[0].r);
